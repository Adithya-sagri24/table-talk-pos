...
export function RMSProvider({ children }: { children: ReactNode }) {
  ...
  const authenticateStaff = useCallback((employeeId: string, pin: string, role: UserRole): { success: boolean; name: string } => {
    ...
    if (cred) {
      addAuditLog({ user: cred.name, role, action: 'PIN login successful', details: `Employee: ${employeeId}, Role: ${role}`, status: 'success' });
      return { success: true, name: cred.name };
    }
    addAuditLog({ user: employeeId, role, action: 'PIN login failed', details: `Attempted role: ${role}`, status: 'failure' });
